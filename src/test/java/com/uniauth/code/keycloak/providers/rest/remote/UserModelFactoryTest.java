...
class UserModelFactoryTest {
    ...
    void setUp() {
        ...
        config.put(ConfigurationProperties.ROLE_MAP_PROPERTY, Arrays.asList("oldRole:newRole"));
        config.put(ConfigurationProperties.GROUP_MAP_PROPERTY, Arrays.asList("oldGroup:newGroup"));
    ...
    void migratesRolesWithoutUnmapped() {
        config.putSingle(ConfigurationProperties.MIGRATE_UNMAPPED_ROLES_PROPERTY, "false");
        final UserProvider userProvider = mock(UserProvider.class);
        final RealmModel realm = mock(RealmModel.class);
        final String username = "user";
        ...
        LegacyUser legacyUser = createLegacyUser(username);
        legacyUser.setRoles(Arrays.asList("oldRole", "anotherRole"));

        UserModel result = userModelFactory.create(legacyUser, realm);

        assertEquals(Stream.of(newRoleModel).collect(Collectors.toSet()), result.getRoleMappingsStream().collect(Collectors.toSet()));
    }

    @Test
    void migratesGroupsWithoutUnmapped() {
        config.putSingle(ConfigurationProperties.MIGRATE_UNMAPPED_GROUPS_PROPERTY, "false");
        final UserProvider userProvider = mock(UserProvider.class);
        final RealmModel realm = mock(RealmModel.class);
        final String username = "user";
        ...
        LegacyUser legacyUser = createLegacyUser(username);
        legacyUser.setGroups(Arrays.asList("oldGroup", "anotherGroup"));

        UserModel result = userModelFactory.create(legacyUser, realm);

        assertEquals(Stream.of(newGroupModel).collect(Collectors.toSet()), result.getGroupsStream().collect(Collectors.toSet()));
    }

    @Test
    void migratesUnmappedRoles() {
        config.putSingle(ConfigurationProperties.MIGRATE_UNMAPPED_ROLES_PROPERTY, "true");
        final UserProvider userProvider = mock(UserProvider.class);
        final RealmModel realm = mock(RealmModel.class);
        final String username = "user";
        final RoleModel newRoleModel = mock(RoleModel.class);
        ...
        assertEquals(Stream.of(newRoleModel, anotherRoleModel).collect(Collectors.toSet()), result.getRoleMappingsStream().collect(Collectors.toSet()));
    }
    ...
    void migrateNotFoundRolesIfEnabled() {
        ...
        String nonPresentRoleName = "thisRoleDoesntYetExistInRealm";
        ...
        LegacyUser legacyUser = createLegacyUser(username);
        legacyUser.setRoles(Arrays.asList(nonPresentRoleName));

        UserModel result = userModelFactory.create(legacyUser, realm);

        assertEquals(Stream.of(roleModel).collect(Collectors.toSet()), result.getRealmRoleMappingsStream().collect(Collectors.toSet()));
    }

    @Test
    void migratesUnmappedGroups() {
        config.putSingle(ConfigurationProperties.MIGRATE_UNMAPPED_GROUPS_PROPERTY, "true");
        final UserProvider userProvider = mock(UserProvider.class);
        final RealmModel realm = mock(RealmModel.class);
        final String username = "user";
        final GroupModel newGroupModel = mock(GroupModel.class);
        ...
        LegacyUser legacyUser = createLegacyUser(username);
        legacyUser.setGroups(Arrays.asList("newGroup", "anotherGroup"));

        UserModel result = userModelFactory.create(legacyUser, realm);

        assertEquals(Stream.of(newGroupModel, anotherGroupModel).collect(Collectors.toSet()), result.getGroupsStream().collect(Collectors.toSet()));
    }
    ...
    void addUserToExistingGroup() {
        ...
        LegacyUser legacyUser = createLegacyUser(username);
        legacyUser.setGroups(Arrays.asList(groupName));
    ...
    void migratesAttribute() {
        ...
        legacyUser.setAttributes(new HashMap<String,List<String>>(){
            {put("someAttribute", Arrays.asList("someValue"));}
        });
        UserModel result = userModelFactory.create(legacyUser, realm);
    ...
    void migratesRequiredActions() {
        ...
        LegacyUser legacyUser = createLegacyUser(username);
        legacyUser.setRequiredActions(Arrays.asList("CONFIGURE_TOTP", "UPDATE_PASSWORD"));

        UserModel result = userModelFactory.create(legacyUser, realm);

        assertEquals(Stream.of("CONFIGURE_TOTP", "UPDATE_PASSWORD").collect(Collectors.toSet()), result.getRequiredActionsStream()
